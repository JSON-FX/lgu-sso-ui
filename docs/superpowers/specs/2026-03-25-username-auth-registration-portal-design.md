...
5. Response returns `{ username, message }` — does NOT return the password in the response

**Success screen shows:**
- Generated username (with copy button)
- Password hint text: "Your default password is your first initial + full last name (e.g., jalanano)" — shows the formula, not the literal password
- Warning that password change will be required on first login
- "Go to Login" button

**Security note:** The default password is not returned in the API response or displayed literally on screen. This avoids exposing credentials in API logs, browser history, and network traces. The pattern-based hint is sufficient since the user just entered their own name.
...
- **Remove the super-admin-only guard:** The current `login()` in `use-auth.ts` throws "Access denied. Super administrator role required." for non-super-admins, and `checkAuth()` forcibly logs out non-super-admins. Both of these guards must be removed and replaced with the role-based redirect logic below.
...
- **Refactor to use shared API layer:** The current SSO login page makes direct `fetch()` calls to the backend API, bypassing `lib/api/` entirely. All API calls on this page — including `/auth/login`, `/sso/validate-redirect`, and `/sso/session-check` — must be refactored to use the shared API client (which routes through mock API when `NEXT_PUBLIC_USE_MOCK_API=true`). This is required for the frontend-first mock development approach to work.
- After successful auth, checks `must_change_password`:
...
2. **New Password** — new password + confirm fields, real-time validation:
   - At least 8 characters
   - Contains a number
   - Contains a special character
   - Note: the user's current (default) password is available in the auth session from the login that just happened. The stepper auto-populates `current_password` in the API call from session context — the user does NOT need to re-enter it.
3. **Done** — success animation, "Continue" button

**On password submit:**
- `POST /auth/change-password` with `{ current_password, new_password }` — `current_password` is sourced from the session, not a form field in the stepper
...
- Employee creation form: remove email and password from required fields in both the form and the `CreateEmployeeData` type — username and default password are auto-generated by the backend. Email becomes an optional profile field.
...
**Critical changes to existing auth logic:**
- Remove the super-admin-only rejection in `login()` (currently throws for non-super-admins)
- Remove the super-admin-only check in `checkAuth()` (currently calls `logout()` for non-super-admins)
- `checkAuth()` must also read `must_change_password` from the user profile and set `mustChangePassword` state accordingly, so that on session restore (browser refresh, tab reopen) the redirect logic re-engages

**Role-based redirect logic (after login and on session restore):**
```
...
|----------|--------|---------|----------|
| `/auth/register` | POST | `{ first_name, middle_name?, last_name }` | `{ username, message }` |
...
| `/portal/profile` | PUT | `UpdatePortalProfileData` | `Employee` |
...
// Portal-specific profile update (subset of employee fields that users can self-edit)
interface UpdatePortalProfileData {
  email?: string
  birthday?: string
